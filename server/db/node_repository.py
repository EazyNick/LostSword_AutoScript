...
class NodeRepository:
    ...
    def get_nodes_by_script_id(self, script_id: int) -> List[Dict]:
        """
        스크립트 ID로 노드 목록 조회
    ...
    def _parse_json_field(self, raw_value: Optional[str], default_value):
        """
        JSON 필드 파싱
    ...
    def build_connections_from_nodes(self, nodes: List[Dict]) -> List[Dict]:
        """
        노드 목록에서 연결 정보 생성
    ...
    def validate_connections(self, nodes: List[Dict], connections: List[Dict]) -> None:
        """
    ...
    def save_nodes(self, script_id: int, nodes: List[Dict], connections: List[Dict]) -> bool:
        """
        노드 저장 (연결 정보 포함)
        ...
        return self.connection.execute_with_connection(
            lambda conn, cursor: self._save_nodes_impl(cursor, script_id, nodes, connections)
        )
    
    def _save_nodes_impl(self, cursor, script_id: int, nodes: List[Dict], connections: List[Dict]) -> bool:
        """노드 저장 구현"""
        # 기존 노드 삭제
    ...
    def cleanup_duplicate_boundary_nodes(self, script_id: int, nodes: List[Dict]) -> List[Dict]:
        """
    ...
    def _cleanup_duplicate_boundary_nodes_impl(self, conn, cursor, script_id: int, nodes: List[Dict]) -> List[Dict]:
        """중복 경계 노드 정리 구현"""
...
if __name__ == "__main__":
    ...
    connections = node_repo.build_connections_from_nodes(retrieved_nodes)
    print(f"✅ 연결 정보 생성 완료")
    print(f"   - 생성된 연결 수: {len(connections)}개")
    for conn in connections:
        output_type = conn.get('outputType', 'None')
        print(f"     * {conn['from']} -> {conn['to']} (outputType: {output_type})")
    print()
