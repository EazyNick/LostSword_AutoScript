...
### 기능 개선
- **프로세스 포커스 노드 UI 개선**: 프로세스 선택 UI 간소화
  - 파라미터 입력란(process_id, hwnd, process_name, window_title) 제거
  - 프로세스 선택 드롭다운만 표시하여 사용자 경험 개선
  - 프로세스 선택 시 자동으로 필요한 값이 설정되어 Python 단에서 처리
- **로그 저장 프로세스 최적화**: 재시도 포함 전체 프로세스를 10초 내에 완료
  - `log_router.py`: 로그 저장 확인 대기 시간 60초 → 10초로 단축, 확인 간격 0.5초 → 0.2초로 단축
  - `log_client.py`: 각 시도 타임아웃 3초 → 2초로 단축, 재시도 간격 0.5초 → 0.3초로 단축
  - `send_log_async`에서 전체 작업(재시도 포함)에 10초 타임아웃 적용
  - 명확한 성공/실패 메시지 제공 (저장 완료/저장 실패 구분)
  - 타임아웃 발생 시에도 최종 확인을 통해 저장 성공/실패 여부 명확히 구분
...
- **`ActionService` 싱글톤 패턴 적용**: `ActionService` 클래스에 싱글톤 패턴을 적용하여 애플리케이션 전체에서 단일 인스턴스만 사용하도록 개선
  - `__new__` 메서드를 통해 인스턴스가 이미 생성되었으면 기존 인스턴스 반환
  - 초기화 중복 방지 플래그(`_initialized`) 추가로 노드 핸들러 스캔 작업이 한 번만 실행되도록 최적화
  - `services/__init__.py`에서 싱글톤 인스턴스를 생성하여 모듈 레벨에서 export
  - `action_router.py`와 `script_router.py`에서 공유 인스턴스 사용으로 메모리 사용량 감소 및 성능 개선
...
### 버그 및 오류 수정
- **프로세스 포커스 노드 파라미터 전달 문제 해결**: `prepareWorkflowData`에서 `nodeManager.nodeData`를 우선 사용하도록 수정
  - DOM에서 가져온 `nodeData` 대신 메모리상의 최신 데이터(`nodeManager.nodeData[nodeId]`)를 우선 사용
  - `actualNodeData` 변수로 통합하여 파라미터 추출 시 올바른 데이터 소스 사용
  - 프로세스 선택 값(process_id, hwnd, process_name, window_title)이 서버로 정상 전달되도록 개선
- **프로세스 포커스 노드 UI 파라미터 폼 중복 생성 문제 해결**: `process-focus` 노드에서 파라미터 폼이 자동 생성되어 입력란이 표시되던 문제 수정
  - `node-settings-modal.js`와 `add-node-modal.js`에서 파라미터 폼 생성 전에 `process-focus` 노드 체크
  - `process-focus` 노드는 파라미터 폼을 건너뛰고 프로세스 선택 UI만 표시
- **로그 저장 확인 메시지 개선**: 타임아웃 발생 시에도 저장 성공/실패 여부를 명확히 구분하도록 메시지 개선
  - 저장 성공: "로그 저장이 완료되었습니다."
  - 저장 실패: "로그 저장이 완료되지 않았습니다. (로그가 저장되지 않음)" 또는 "로그 저장이 완료되지 않았습니다. (예상 상태와 일치하지 않음)"
...
### 린팅 및 타입 체크 개선
- **타입 어노테이션 추가**: `process_focus_models.py`와 `action_node_types.py`에 누락된 타입 어노테이션 추가
  - Pydantic `field_validator` 메서드의 `cls` 파라미터 타입 명시 (`type["ProcessFocusParams"]`)
  - `model_post_init` 메서드의 `self` 파라미터 타입 명시 (`"ProcessFocusParams"`)
  - `model_post_init` 메서드의 `__context` 파라미터 타입 명시 (`Any`)
  - `validate_values` 메서드의 `v` 파라미터 타입 명시 (`Any`)
  - `ACTION_NODE_TYPES` 변수에 타입 어노테이션 추가 (`dict[str, dict[str, Any]]`)
- **사용하지 않는 import 제거**: `process_focus_models.py`에서 사용하지 않는 `ClassVar` import 제거
