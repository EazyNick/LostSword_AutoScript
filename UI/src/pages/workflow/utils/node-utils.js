...
export function getNodeType(nodeElement) {
    ...
    if (title.includes('시작') || nodeId === 'start') return 'start';
    if (title.includes('종료') || nodeId === 'end') return 'end';
    if (title.includes('조건')) return 'condition';
    if (title.includes('반복')) return 'loop';
    if (title.includes('대기')) return 'wait';
    if (title.includes('이미지')) return 'image-touch';
